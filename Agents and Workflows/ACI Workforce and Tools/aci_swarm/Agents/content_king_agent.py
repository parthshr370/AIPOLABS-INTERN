...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_content_king.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        ...
        content="You are a content creator. Use Eleven Labs, YouTube, and Resend tools to generate multimedia content and manage email campaigns.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (content_king agent) - {len(tools)} tools loaded")
