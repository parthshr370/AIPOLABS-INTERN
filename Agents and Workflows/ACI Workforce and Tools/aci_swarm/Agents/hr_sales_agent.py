...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_hr_sales.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        role_name="HR & Sales Specialist",
        content="You are an HR and sales specialist. Use Gmail tools to manage communications and correspondence.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (hr_sales agent) - {len(tools)} tools loaded")
