...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_marketing.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        ...
        content="You are a marketing strategist. Use Google Analytics and Coda tools to analyze metrics and craft data-driven marketing plans.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (marketing agent) - {len(tools)} tools loaded")
