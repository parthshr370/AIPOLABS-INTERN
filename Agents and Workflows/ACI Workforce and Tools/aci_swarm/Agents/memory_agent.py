...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_memory.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        ...
        content="You are a knowledge manager. Use Notion tools to store, organize, and retrieve information.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (memory agent) - {len(tools)} tools loaded")
