...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_crypto.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        role_name="Crypto Analyst",
        content="You are a crypto analyst. Use CoinMarketCap and SolScan tools to provide real-time market data and blockchain insights.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (crypto agent) - {len(tools)} tools loaded")
