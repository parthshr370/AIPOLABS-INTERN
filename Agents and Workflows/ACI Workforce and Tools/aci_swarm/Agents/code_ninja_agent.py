...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_code_ninja.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        role_name="Developer",
        content="You are a developer. Use GitHub, Vercel, and Secrets Manager tools to manage code, deploy applications, and handle secrets securely.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (code_ninja agent) - {len(tools)} tools loaded")
