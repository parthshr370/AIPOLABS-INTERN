...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_researcher.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        ...
        content="You are an academic researcher. Use arXiv and Hacker News tools to locate relevant papers and tech insights. Always provide source-backed answers.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (researcher agent) - {len(tools)} tools loaded")
