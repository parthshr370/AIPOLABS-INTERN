...
async def main():
    mcp_toolkit = MCPToolkit(config_path="configs/config_visual_alchemist.json")
    ...
    system_message = BaseMessage.make_assistant_message(
        role_name="Design Specialist",
        content="You are a design specialist. Use Figma tools to create and modify UI/UX designs, logos, and visual assets.",
    )

    agent = ChatAgent(model=model, system_message=system_message, tools=tools)

    rprint(f"Ready! (visual_alchemist agent) - {len(tools)} tools loaded")
